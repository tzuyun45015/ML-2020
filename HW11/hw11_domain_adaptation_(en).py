...
#lambda1 = lambda epoch: 1/((1+10*epoch)**0.75) 
#scheduler1 = torch.optim.lr_scheduler.LambdaLR(optimizer_F, lr_lambda = lambda1)

optimizer_C = optim.Adam(label_predictor.parameters())
#lambda2 = lambda epoch: 1/((1+10*epoch)**0.75) 
#scheduler2 = torch.optim.lr_scheduler.LambdaLR(optimizer_C, lr_lambda = lambda1)

optimizer_D = optim.Adam(domain_classifier.parameters())
#lambda3 = lambda epoch: 1/((1+10*epoch)**0.75) 
#scheduler3 = torch.optim.lr_scheduler.LambdaLR(optimizer_D, lr_lambda = lambda1)
...
for epoch in range(1000):
    ...
    if epoch == 50:
        torch.save(feature_extractor.state_dict(), f'early_model.bin')
    if epoch == 400:
        torch.save(feature_extractor.state_dict(), f'middle_model.bin')
    if epoch == 999:
        torch.save(feature_extractor.state_dict(), f'final_model.bin')
...
df.to_csv('DaNN_submission6.csv',index=False)
...
X = []
feature_extractor.eval()
feature_extractor.load_state_dict(torch.load("extractor_model.bin")) #early_model #middle_model.bin #final_model.bin 
for i, (source_data, source_label) in enumerate(source_dataloader):
    ...
    if i ==0:
        ...
        s_label = torch.cat([s_label,source_label])
...
source = source_feature.cpu().detach().numpy()
source_tsne = manifold.TSNE(n_components=2, init='random', random_state=5, verbose=1).fit_transform(source)

# Normalization the processed features 
x_min, x_max = source_tsne.min(0), source_tsne.max(0)
source_norm = (source_tsne - x_min) / (x_max - x_min)
...
for i in range(source_norm.shape[0]):
    plt.text(source_norm[i,0],source_norm[i,1],str(s_label[i]),
            color = plt.cm.Set1(s_label[i]))   
...
for i, ((source_data, source_label), (target_data, _)) in enumerate(zip(source_dataloader, target_dataloader)):
    ...
    source_data = source_data.cuda()
    source_label = source_label.cuda()
    target_data = target_data[:source_data.shape[0]].cuda()
    ...
    if i ==0:
        ...
        t_label = torch.cat([t_label,domain_label],dim = 0)
...
target = target_feature.cpu().detach().numpy()
target_tsne = manifold.TSNE(n_components=2, init='random', random_state=5, verbose=1).fit_transform(target)

# Normalization the processed features 
x_min, x_max = target_tsne.min(0), target_tsne.max(0)
target_norm = (target_tsne - x_min) / (x_max - x_min)
t_label = t_label.cpu().detach().numpy()
...
for i in range(source_norm.shape[0]):
    plt.scatter(source_norm[i,0],source_norm[i,1],s_label[i],
            color = plt.cm.Set1(source_norm[0,1]))
    plt.scatter(target_norm[i,0],target_norm[i,1],t_label[i],
            color = plt.cm.Set1(target_norm[0,1]))
